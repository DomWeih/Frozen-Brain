...
public class CameraManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        zoomStrategy = cam.orthographic ? new OrthographicZoomStrategy(cam, startingZoom) : new PerspectiveZoomStrategy(cam, cameraOffset, startingZoom);
        cam.transform.LookAt(transform.position + Vector3.up * lookAtOffset);
    }

    private void OnEnable()
    {
        KeybboardInputManager.OnMoveInput += UpdateFrameMove;
        KeybboardInputManager.OnRotateInput += UpdateFrameRotate;
        KeybboardInputManager.OnZoomInput += UpdateFrameZoom;
